...
-   `rate` rate parameter of exponential dispersal kernel

-   `pattern`	fragmentation pattern; either "random" (random location of fragmentation), "clustered" (clustered around the initial fragmentation that is randomly placed), "upstream" (upstream edges are more likely to be fragmented), or "downstream" (downstream edges are more likely to be fragmented).
...
Just like `ptsource()`, this function can be used with an output from `brnet()` function but also accepts any adjacency matrix.
